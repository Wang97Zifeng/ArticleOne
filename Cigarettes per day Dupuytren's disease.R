###CigarettesPerDay Dupuytren's disease
getwd()
setwd('E:/RWork')
...
exposure_data<-fread("CigarettesPerDay.txt",header = T)
...
dat$exposure <- "Cigarettes Per Day"
...
dat$samplesize.exposure <- 337334	  
...
#Leave-one-out(Supplementary Figure 7. Leave-one-out analysis for cigarettes per day.)
single <- mr_leaveoneout(dat)
write.csv( single, file="single.csv")
mr_leaveoneout_plot( single)

#Scatter plot(Supplementary Figure 2. Scatter plot for cigarettes per day.)
mr_scatter_plot(res,dat)

#Funnel plot(Supplementary Figure 8. Funnel plot for cigarettes per day.)
