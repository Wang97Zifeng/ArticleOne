###Smoking Cessation Dupuytren's disease
getwd()
setwd('E:/RWork')
...
exposure_data<-fread("SmokingCessation.txt",header = T)
...
dat$exposure <- "Smoking Cessation"
...
dat$samplesize.exposure <- 547219 	  
...
#Leave-one-out(Supplementary Figure 8. Leave-one-out analysis for smoking cessation.)
single <- mr_leaveoneout(dat)
write.csv( single, file="single.csv")
mr_leaveoneout_plot( single)

#Scatter plot(Supplementary Figure 4. Scatter plot for smoking cessation.)
mr_scatter_plot(res,dat)

#Funnel plot(Supplementary Figure 10. Funnel plot for smoking cessation.)
