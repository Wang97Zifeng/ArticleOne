#Smoking Initiation Dupuytren's disease
getwd()
setwd('E:/RWork')
...
exposure_data<-fread("SmokingInitiation.txt",header = T)
...
dat$samplesize.exposure <-  1232091	  
...
#Leave-one-out(Supplementary Figure 9. Leave-one-out analysis for smoking initation.)
single <- mr_leaveoneout(dat)
write.csv( single, file="single.csv")
mr_leaveoneout_plot( single)

#Scatter plot(Supplementary Figure 5. Scatter plot for smoking initiation.)
mr_scatter_plot(res,dat)

#Funnel plot(Supplementary Figure 11. Funnel plot for smoking initation.)
